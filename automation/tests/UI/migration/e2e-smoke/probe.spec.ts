...
test('build migration registry from API', async ({ request }) => {
  ...
  const results = (listings as any[]).map((l: any) => {
    const entities: any[] = l.entities || [];
    const isFile = entities.some((e: any) => e.fromImportV2 === true);
    const dataTypes = [...new Set(entities.map((e: any) => e.entityName).filter(Boolean))];
    return {
      name: l.name,
      id:   l.id,
  ...
  const files = unique.filter((r: any) => r.type === 'file');
  const apis  = unique.filter((r: any) => r.type === 'api');
  ...
  unique.forEach((r: any) => {
    const dt = r.dataTypes.map((d: string) => `'${d}'`).join(', ');

...
import config_utils from '../../../../utils/config';
 ...
 */
async function detectNextStep(page: Page, wizard: MigrationWizardPage) {
  const TIMEOUT = 20000;
  const checks = [
 ...
 */
async function handleFileUpload(page: Page, wizard: MigrationWizardPage, config: any) {
  ...
  if (config.dataTypes.length === 1) {
    ...
    for (const type of config.dataTypes) {
      ...
      const hasUploadBtn = await sectionWithUpload.count().then((n: number) => n > 0).catch(() => false);
 ...
 */
async function connectOAuthSource(page: Page, sourceType: string, email: string | undefined, password: string | undefined) {
  console.log(`[runner] Step 2 — OAuth source connect: ${sourceType}`);
  const waitVis = (loc: Locator, ms: number) => loc.waitFor({ state: 'visible', timeout: ms }).then(() => true).catch(() => false);
 ...
 */
async function handleSourceConnect(page: Page, wizard: MigrationWizardPage, config: any) {
 ...
 */
async function connectOAuthDestination(page: Page, wizard: MigrationWizardPage, targetType: string, email: string | undefined, password: string | undefined) {
 ...
 */
async function connectShopifyDestination(page: Page, wizard: MigrationWizardPage, shop: string, email: string | undefined, password: string | undefined) {
 ...
 */
async function handleDestConnect(page: Page, wizard: MigrationWizardPage, config: any) {
 ...
 */
async function handleSettings(page: Page, wizard: MigrationWizardPage) {
...
async function handleReview(page: Page, wizard: MigrationWizardPage, dryRun = false) {
 ...
 */
async function advanceToReview(page: Page, wizard: MigrationWizardPage) {
 ...
 * @param config  one entry from migrations.config.ts
 * @returns final page URL after migration launched
 */
export async function runE2eMigration(page: Page, config: any): Promise<string> {

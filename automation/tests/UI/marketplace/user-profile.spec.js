...
test.describe('User Profile / Account Menu', () => {
  ...
  test('user menu button is visible and displays the logged-in email', async ({ page }) => {
    ...
    expect(label.trim()).toMatch(/@/, 'User menu button should display the logged-in email address');
  });
  ...
  test('profile or settings navigation exists if page is available', async ({ page }) => {
    ...
    if (profileVisible) {
      ...
      try {
        ...
        console.log('[user-profile] Profile link click failed (false positive or overlay):', e.message.split('\n')[0]);
